...
void GrowBallWorld::spawn_ai(int index, uint32_t now_ms) {
    ...
    for (int retry = 0; retry < AI_SPAWN_RETRIES; ++retry) {
...
void GrowBallWorld::update_ai(uint32_t now_ms) {
    for (int i = 0; i < AI_COUNT; ++i) {
        ...
        auto check_target = [&](float tx, float ty, float tr) {
            ...
            if (dist < AI_DETECT_RANGE && ai.radius > tr * AI_DETECT_SIZE_RATIO && dist < nearest_smaller_dist) {
                nearest_smaller_dist = dist;
                smaller_dx = dx;
                smaller_dy = dy;
            }
            if (dist < AI_DETECT_RANGE && tr > ai.radius * AI_DETECT_SIZE_RATIO && dist < nearest_larger_dist) {
        ...
        // 逃跑优先：先于追击判断，确保附近有更大球时优先逃离
        if (nearest_larger_dist < AI_FLEE_RANGE) {
            target_dx = -larger_dx;
            target_dy = -larger_dy;
        } else if (nearest_smaller_dist < AI_CHASE_RANGE) {
...
void GrowBallWorld::resolve_collisions(uint32_t now_ms, bool& victory, bool& game_over) {
    ...
    for (int i = 0; i < AI_COUNT; ++i) {
        for (auto& f : _foods) {
            ...
            if (dist < _ai[i].radius + f.radius && _ai[i].radius > f.radius) {
                _ai[i].radius = add_area_growth(_ai[i].radius, FOOD_MASS * AI_FOOD_GROWTH);
                spawn_food(static_cast<int>(&f - _foods));
            }
        }
    }

    // AI vs AI — 双方均可互食时以尺寸大者优先，避免低索引偏差
    for (int i = 0; i < AI_COUNT; ++i) {
        for (int j = i + 1; j < AI_COUNT; ++j) {
            ...
            if (dist < overlap && dist > 0.01F) {
                bool const i_eats_j = _ai[i].radius > _ai[j].radius * AI_EAT_RATIO;
                bool const j_eats_i = _ai[j].radius > _ai[i].radius * AI_EAT_RATIO;

                if (i_eats_j && j_eats_i) {
                    // 双方均可互食 → 尺寸大者胜
                    if (_ai[i].radius >= _ai[j].radius) {
                        _ai[i].radius = add_area_growth(_ai[i].radius, _ai[j].radius * _ai[j].radius * AI_VS_AI_GROWTH);
                        spawn_ai(j, now_ms);
                    } else {
                        _ai[j].radius = add_area_growth(_ai[j].radius, _ai[i].radius * _ai[i].radius * AI_VS_AI_GROWTH);
                        spawn_ai(i, now_ms);
                    }
                } else if (i_eats_j) {
                    _ai[i].radius = add_area_growth(_ai[i].radius, _ai[j].radius * _ai[j].radius * AI_VS_AI_GROWTH);
    ...
    for (int i = 0; i < AI_COUNT; ++i) {
        ...
        if (dist < (_player.radius + _ai[i].radius) * PLAYER_VS_AI_OVERLAP) {
            ...
            } else if (_ai[i].radius > _player.radius * AI_EAT_PLAYER_SIZE_RATIO) {
                game_over = true;
                return;
            }
        }
