    ...
    for (auto& g : _ghosts) {
        ...
        if (vcount > 1) {
            if (g.state == GhostState::VULNERABLE) {
                ...
                int8_t tx = ghost_target_x(g), ty = ghost_target_y(g);
                int8_t best_dist = 99;
...
void PacmanScreen::die() {
    ...
    int16_t px = static_cast<int16_t>(_pac_x * CELL + CELL / 2);
    int16_t py = static_cast<int16_t>(_pac_y * CELL + CELL / 2);
...
void PacmanScreen::render(IPlatform& platform, IScreenHost& /*host*/) {
    ...
    for (const auto& g : _ghosts) {
        ...
        int16_t ex1_x = gx + 2, ex1_y = gy + 2, ex2_x = gx + 5, ex2_y = gy + 2;
        switch (g.dir) {
    ...
    if (_celebration_timer > 0) {
        for (uint8_t i = 0; i < 5; ++i) {
            int16_t sx = static_cast<int16_t>(next_rng() % 80);
            int16_t sy = static_cast<int16_t>(next_rng() % 80);
