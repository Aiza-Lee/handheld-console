...
void MenuScreen::init_stars() {
    ...
    for (int16_t i = 0; i < STAR_COUNT; ++i) {
        _stars[i].x = static_cast<int16_t>(next_rng() % 80);
        _stars[i].y = static_cast<int16_t>(next_rng() % 80);
        _stars[i].speed = static_cast<uint8_t>(1 + (next_rng() % 2));
        _stars[i].layer = static_cast<uint8_t>(next_rng() % 2);
    }
...
void MenuScreen::update_stars() {
    for (int16_t i = 0; i < STAR_COUNT; ++i) {
        ...
        if (_stars[i].y < 0) { _stars[i].y = 79; _stars[i].x = static_cast<int16_t>(next_rng() % 80); }
    }
}
...
void MenuScreen::update(IPlatform& platform, IScreenHost& host) {
    ...
    if (_cursor >= _scroll_offset + MAX_VISIBLE) _scroll_offset = _cursor - MAX_VISIBLE + 1;
}
...
void MenuScreen::render(IPlatform& platform, IScreenHost& /*host*/) {
    ...
    for (int16_t i = 0; i < STAR_COUNT; ++i)
        display.draw_pixel(_stars[i].x, _stars[i].y, (_stars[i].layer == 0) ? MENU_STAR_DIM : MENU_STAR_BRIGHT);
