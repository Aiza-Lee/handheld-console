...
const char* _mode_name(uint8_t mode) {
    switch (mode) {
        case 0: return "GLASS GARDEN";
        case 1: return "LUNAR WALTZ";
        case 2: return "DREAM TIDE";
        case 3: return "SILK WAVE";
        case 4: return "AURORA BLOOM";
...
void PlaygroundScreen::update(IPlatform& platform, IScreenHost& host) {
    ...
    if (input.was_pressed(ButtonBits::A)) {
        ...
        host.audio().play_sfx(_auto_move ? sounds::SFX_TOGGLE_ON : sounds::SFX_TOGGLE_OFF,
                              _auto_move ? sounds::SFX_TOGGLE_ON_COUNT : sounds::SFX_TOGGLE_OFF_COUNT);
    }

    if (input.was_pressed(ButtonBits::B)) {
        _clear_each_frame = !_clear_each_frame;
        host.audio().play_sfx(_clear_each_frame ? sounds::SFX_TOGGLE_ON : sounds::SFX_TOGGLE_OFF,
                              _clear_each_frame ? sounds::SFX_TOGGLE_ON_COUNT : sounds::SFX_TOGGLE_OFF_COUNT);
    }
...
void PlaygroundScreen::render(IPlatform& platform, IScreenHost& /*host*/) {
    ...
    switch (_mode) {
        case 0: render_mode_grid_tunnel(display); break;
        case 1: render_mode_orbit_lab(display); break;
        case 2: render_mode_starflow(display); break;
        case 3: render_mode_signal_scope(display); break;
        case 4: render_mode_aurora_bloom(display); break;
        default: break;
    }
    render_hud(display);
    display.draw_rect(Rect{1, 1, static_cast<int16_t>(display.width() - 2), static_cast<int16_t>(display.height() - 2)},
                      MIST);
}
...
void PlaygroundScreen::render_stage_shell(IDisplay& display) const {
    ...
    display.draw_h_line(static_cast<int16_t>(stage.x + stage.width - wing),
                        static_cast<int16_t>(stage.y + stage.height - 1), wing, HIGHLIGHT);
}
...
void PlaygroundScreen::update_starfield(IDisplay& display) {
    ...
    for (size_t i = 0; i < _stars.size(); ++i) {
        ...
        if (s.y >= stage.bottom()) {
            s.y = stage.y;
            s.x = static_cast<int16_t>(stage.x +
                                       (hash_u32(_frame + static_cast<uint32_t>(i * 31)) % static_cast<uint32_t>(w)));
        }
...
void PlaygroundScreen::render_mode_orbit_lab(IDisplay& display) const {
    ...
    for (int16_t r = 6; r < static_cast<int16_t>((stage.height / 2) - 4); r += 8) {
        display.draw_rect(Rect{static_cast<int16_t>(center.x - r), static_cast<int16_t>(center.y - r),
                               static_cast<int16_t>(r * 2), static_cast<int16_t>(r * 2)},
                          (r % 16 == 0) ? PRIMARY : MIST);
    }

    const double t = static_cast<double>(_frame) * 0.058;
    const Point sat_a = {static_cast<int16_t>(center.x + (cos_lut_double(phase_from_radians(t)) * 34.0)),
                         static_cast<int16_t>(center.y + (sin_lut_double(phase_from_radians(t)) * 20.0))};
    const Point sat_b = {static_cast<int16_t>(center.x + (cos_lut_double(phase_from_radians(-t * 1.6)) * 22.0)),
                         static_cast<int16_t>(center.y + (sin_lut_double(phase_from_radians(-t * 1.6)) * 12.0))};
    const Point sat_c = {static_cast<int16_t>(center.x + (cos_lut_double(phase_from_radians(t * 0.7)) * 14.0)),
...
void PlaygroundScreen::render_mode_starflow(IDisplay& display) const {
    ...
    for (int16_t x = stage.x; x < static_cast<int16_t>(stage.x + stage.width); x += 4) {
        const double wave =
            sin_lut_double(phase_from_radians((static_cast<double>(x) * 0.12) + (static_cast<double>(_frame) * 0.09)));
        const auto y = static_cast<int16_t>(horizon + (wave * 3.0));
        display.draw_pixel(x, y, MIST);
    }

    for (auto lane = static_cast<int16_t>(stage.x + 6); lane < static_cast<int16_t>(stage.x + stage.width);
         lane += 28) {
        display.draw_line(stage_center(display).x, static_cast<int16_t>(stage.y + stage.height - 1), lane, horizon,
                          SECONDARY);
    }

    const auto ship_y = static_cast<int16_t>(stage.y + stage.height - 22);
    const int16_t ship_x =
        _clamp_i16(_x, static_cast<int16_t>(stage.x + 8), static_cast<int16_t>(stage.x + stage.width - 9));
    ...
    const auto meteor_x =
        static_cast<int16_t>(stage.x + stage.width - 1 - ((_frame * 2U) % static_cast<uint32_t>(stage.width + 16)));
    ...
    if (meteor_x >= stage.x && meteor_x < static_cast<int16_t>(stage.x + stage.width)) {
        display.draw_line(meteor_x, meteor_y, static_cast<int16_t>(meteor_x + 6), static_cast<int16_t>(meteor_y - 2),
                          ALERT);
...
void PlaygroundScreen::render_mode_signal_scope(IDisplay& display) const {
    ...
    for (auto x = static_cast<int16_t>(stage.x + 3); x < static_cast<int16_t>(stage.x + stage.width - 2); ++x) {
        ...
        const double wave =
            (sin_lut_double(phase_from_radians(t)) * 7.0) + (sin_lut_double(phase_from_radians(t * 0.37)) * 3.0);
...
void PlaygroundScreen::render_mode_aurora_bloom(IDisplay& display) const {
    ...
    for (int16_t k = 0; k < 9; ++k) {
        ...
        display.draw_line(center.x, center.y, static_cast<int16_t>(center.x + dx), static_cast<int16_t>(center.y + dy),
                          petal);
...
void PlaygroundScreen::render_hud(IDisplay& display) const {
    ...
    TextRenderer::draw_text(display, {static_cast<int16_t>(display.width() - 34), hint_y}, "TRC", clear_color, 1,
                            COMPACT_FONT_3X5);
}
...
void PlaygroundScreen::render_menu_preview(IDisplay& display, const Rect& box, uint32_t frame) {
    const auto cx = static_cast<int16_t>(box.x + box.width / 2);
    const auto cy = static_cast<int16_t>(box.y + box.height / 2);
    const Color dot = rgb565(180, 200, 240);
    double angle = static_cast<double>(frame) * 0.12;
    for (int i = 0; i < 4; ++i) {
        double a = angle + i * 1.5708;
        auto px = static_cast<int16_t>(cx + cos_lut_double(phase_from_radians(a)) * 2.0);
        auto py = static_cast<int16_t>(cy + sin_lut_double(phase_from_radians(a)) * 2.0);
        display.draw_pixel(px, py, dot);
    }
}
