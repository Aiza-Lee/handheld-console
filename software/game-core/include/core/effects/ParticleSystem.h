...
public:
    ...
    void emit(int8_t x, int8_t y, int8_t vx, int8_t vy, uint8_t life, Color color) {
        ...
        for (size_t i = 0; i < _count; ++i) {
            if (_parts[i].life == 0) {
                _parts[i].x = x; _parts[i].y = y;
                _parts[i].vx = vx; _parts[i].vy = vy;
                _parts[i].life = life;
        ...
        // 未达上限则新增
        if (_count < N) {
            _parts[_count].x = x; _parts[_count].y = y;
            _parts[_count].vx = vx; _parts[_count].vy = vy;
    ...
    void render(IDisplay& display) const {
        for (size_t i = 0; i < _count; ++i) {
            if (_parts[i].life > 0)
                display.draw_pixel(_parts[i].x, _parts[i].y, _parts[i].color);
        }
    }
