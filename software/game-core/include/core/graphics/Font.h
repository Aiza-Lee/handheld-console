...
inline constexpr GlyphBitmap GLYPH_PERCENT = {0x41, 0x22, 0x14, 0x08, 0x14};
...
[[nodiscard]] constexpr PackedGlyph3x5 pack_glyph_3x5(uint8_t column0, uint8_t column1, uint8_t column2) {
    return static_cast<PackedGlyph3x5>((column0 & 0x1F) | ((column1 & 0x1F) << 5) | ((column2 & 0x1F) << 10));
}
...
inline constexpr PackedGlyph3x5 COMPACT_GLYPH_PERCENT = pack_glyph_3x5(0x11, 0x0A, 0x00);
...
[[nodiscard]] constexpr GlyphBitmap compact_glyph_3x5(char c) {
    switch (normalize_glyph_char(c)) {
        ...
        case '%': return unpack_glyph_3x5(COMPACT_GLYPH_PERCENT);
...
struct BitmapFont {
    ...
    [[nodiscard]] constexpr detail::GlyphBitmap glyph_for(char c) const { return glyph_lookup(c); }
};
