...
public:
    ...
    [[nodiscard]] ButtonState read_buttons() const { return current_buttons(); }

    [[nodiscard]] bool is_down(ButtonBits button) const { return current_buttons().test(button); }
    ...
    [[nodiscard]] bool changed() const { return current_buttons() != previous_buttons(); }
};
