...
public:
    ...
    [[nodiscard]] uint16_t read_battery_millivolts() const { return read_status().batteryMilliVolts; }

    [[nodiscard]] uint8_t read_battery_percent() const { return read_status().batteryPercent; }

    [[nodiscard]] bool is_charging() const { return read_status().charging; }
};
